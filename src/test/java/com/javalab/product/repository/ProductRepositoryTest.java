...
//	}
	...
	public void testProductSave() {
	    ...
	    // Search for a category object that actually exists to be used when registering a product
	    Integer categoryId = 1;
	    Category category = categoryRepository.findById(categoryId).orElseThrow(); // Retrieve an existing Category entity from the database
	    ...
	    IntStream.rangeClosed(101, 200).forEach(i -> {
	        ...
	        // Persistence (subject to management by entity manager in memory)
	        productRepository.save(product);
	    });
	 ...
	 * 
	 * [Fetch 전략 - Eager / Lazy]
	 * - Fecth 전략을 Lazy로 하게 되면 지연로딩이 되어 처음에는
	 ...
	 *   사용하는게 낫다.
	 *  - @ManyToOne : 기본이 지연로딩(Eager)
	 *  - @OneToMay : 기본이 즉시로딩(Lazy)
	 */
	//@Test
	//@Disabled
	...
	// 1. 상품 엔티티의 Category 멤버 변수에ㅐ 지연로딩을 설정해놓게
	// 	  되면 이 시점 까지는 상품 테이블만 조회함.
	...
	public void testGetProductById2() {
		Optional<Product> optProduct = productRepository.findById(50); // 데이터베이스 확인
		Product product = optProduct.get();
	...
	public void testProductUpdate() {
		...
		Product product = new Product();
		product.setProductId(77);	// 실존하는 상품Id으로
